  ...
  <div class="p-2 w-full">
    <div class="font-extrabold flex items-center justify-between mt-0.5 mb-1.5">
      <div class="flex items-center">
        <div>{{ tweet.name }}</div>
        <span class="font-[300] text-[15px] pl-2 text-gray-500">{{
          tweet.handle
        }}</span>
      ...
      <div class="hover:bg-gray-800 rounded-full cursor-pointer relative">
        ...
        <div
          v-if="openOptions"
          class="absolute mt-1 p-3 right-0 bg-black border border-gray-700 rounded-lg shadow-lg w-[300px]"
        >
          <div class="flex" as="button" @click="deleteTweet(tweet.ID)">
    ...
    <div class="flex items-center justify-between mt-4 w-4/5">
      <div class="flex">
        ...
        <span class="text-xs text-[#5e5c5c] font-extrabold ml-3">{{
          tweet.comments
        }}</span>
      </div>
      <div class="flex">
